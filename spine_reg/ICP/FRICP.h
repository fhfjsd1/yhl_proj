...
#pragma omp parallel for
    ...
    AffineNd point_to_point(Eigen::MatrixBase<Derived1>& X,
        ...
        //std::cout << svd.matrixV() * svd.matrixU().transpose() << std::endl;
        //std::cout << std::endl << svd.matrixV() << std::endl;
...
#pragma omp parallel for
        ...
        end_time = omp_get_wtime();
        double registration_time = end_time - begin_time;
        ...
        par.init_time = init_time;
        par.registration_time = registration_time;
        par.iter_num = iter_num;
    ...
    void point_to_point(MatrixNX& X, MatrixNX& Y, MatrixNX& X1, MatrixNX& X2, MatrixNX& X3, MatrixNX& Y1, MatrixNX& Y2, MatrixNX& Y3, VectorN& source_mean,
                        VectorN& target_mean, ICP::Parameters& par){
        double begin_init = omp_get_wtime();
        /// Build kd-tree
        KDtree kdtree(Y);
...
#pragma omp parallel for
  ...
  //          test << Q.col(i)[0] << " " << Q.col(i)[1] << " " << Q.col(i)[2] << std::endl;
  //      }
  //      test.close();

        //std::string path = "my_test_result.txt";
        //std::ofstream test(path);
        //for (int i = 0; i < W.rows(); i++) {
        //    test << W.row(i).transpose() << std::endl;
        //}
        //test.close();
        ...
        //std::ofstream test(path);
        //for (int i = 0; i < Q.cols(); i++) {
        //    test << Q.col(i).transpose() << std::endl;
        //}
        //test.close();
        // 这一步输出 Q （X的对应Y最近点序列） 和 W （最近点X和Q之间的距离）
