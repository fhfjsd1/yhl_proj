///////////////////////////////////////////////////////////////////////////////
///   “稀疏迭代最近点 (Sparse Iterative Closest Point)”
///   作者: Sofien Bouaziz, Andrea Tagliasacchi, Mark Pauly
///   版权 (C) 2013  LGG, EPFL
///////////////////////////////////////////////////////////////////////////////
///   1) 本文件包含 ICP 算法的多种实现。
///   2) 本代码依赖 EIGEN 和 NANOFLANN。
///   3) 若启用 OPENMP，部分代码将并行化执行。
///   4) 本代码目前用于 3D 配准。
///   5) 两种主要输入类型：Eigen::Matrix3Xd 或 Eigen::Map<Eigen::Matrix3Xd>。
///////////////////////////////////////////////////////////////////////////////
///   namespace nanoflann: 为 EIGEN 提供的 NANOFLANN KD 树适配器
///   namespace RigidMotionEstimator: 刚性运动计算函数
///   namespace SICP: 稀疏 ICP 实现
///   namespace ICP: 重加权 ICP 实现
///////////////////////////////////////////////////////////////////////////////
#ifndef ICP_H
#define ICP_H
...
namespace nanoflann
{
    /// 用于直接使用存储在 Eigen 矩阵中的数据而不复制数据存储的 KD 树适配器。
    /// 此代码改编自 nanoflann.hpp 中的 KDTreeEigenMatrixAdaptor 类
...
namespace ICP
{
    ...
    public:
        Parameters()
            : f(NONE) // 鲁棒函数类型，默认为 NONE
              ,
              p(0.1) // 鲁棒函数参数 p
              ,
              max_icp(100) // 最大 ICP 迭代次数
              ,
              max_outer(1) // 每次 ICP 的最大外部迭代次数
              ,
              stop(1e-5) // 收敛停止阈值
              ,
              use_AA(false) // 是否使用 Anderson 加速
              ,
              print_energy(false) // 是否打印能量值
              ,
              print_output(false) // 是否将结果写入文件
              ,
              anderson_m(5) // Anderson 加速的 m 参数
              ,
              beta_(1.0) // Anderson 加速中的 β 参数
              ,
              error_overflow_threshold_(0.05) // 能量溢出阈值
              ,
              has_groundtruth(false) // 是否提供地面真实变换
              ,
              gt_trans(Eigen::Matrix4d::Identity()) // 地面真实变换矩阵（4×4 单位矩阵）
              ,
              convergence_energy(0.0) // 最终收敛能量
              ,
              convergence_iter(0) // 终止时迭代次数
              ,
              convergence_gt_mse(0.0) // 最终地面真 MSE
              ,
              nu_begin_k(3) // 对数映射初始步长 k
              ,
              nu_end_k(1.0 / (3.0 * sqrt(3.0))) // 对数映射结束步长 k
              ,
              use_init(false) // 是否使用用户提供的初始变换
              ,
              init_time(0) // 初始化耗时
              ,
              registration_time(0) // 注册过程耗时
              ,
              iter_num(0) // 总迭代计数
              ,
              nu_alpha(1.0 / 2) // 对数映射调节因子 α
        {}
        /// Parameters
    ...
    double tukey_energy(Eigen::VectorXd &r, double p)
    {
        ...
        for (int i = 0; i < r.rows(); ++i)
        {
            if (r(i) > p)
                ...
                w = std::pow((1.0 - std::pow(r(i) / p, 2.0)), 2.0);
    ...
    double get_energy(Function f, Eigen::VectorXd &r, double p)
    {
        ...
        case TUKEY:
            energy = tukey_energy(r, p);
            break;
        case FAIR:
            energy = fair_energy(r, p);
            break;
        case LOGISTIC:
            energy = logistic_energy(r, p);
            break;
        case TRIMMED:
            energy = trimmed_energy(r, p);
            break;
        case WELSCH:
            energy = welsch_energy(r, p);
            break;
        case AUTOWELSCH:
            energy = autowelsch_energy(r, p);
            break;
        case NONE:
            energy = uniform_energy(r);
            break;
        default:
            energy = uniform_energy(r);
            break;
...
namespace AAICP
{
    ...
    {
        Matrix3 s(Eigen::AngleAxis<Scalar>(v(0), Vector3::UnitX()) * Eigen::AngleAxis<Scalar>(v(1), Vector3::UnitY()) * Eigen::AngleAxis<Scalar>(v(2), Vector3::UnitZ()));
