 ...
 * 软件许可协议（BSD 许可证）
 *
 * 版权所有 2008-2009  Marius Muja (mariusm@cs.ubc.ca)。保留所有权利。
 * 版权所有 2008-2009  David G. Lowe (lowe@cs.ubc.ca)。保留所有权利。
 * 版权所有 2011  Jose Luis Blanco (joseluisblancoc@gmail.com)。保留所有权利。
 *
 * 本 BSD 许可证
 *
 * 源代码和二进制形式的再分发与使用，是否经过修改，均被允许，但须满足以下条件：
 *
 * 1. 源代码再分发时必须保留上述版权声明、本条款列表以及以下免责声明。
 * 2. 二进制形式再分发时必须在随附的文档和/或其他资料中重现上述版权声明、
 *    本条款列表以及以下免责声明。
 *
 * 本软件按“原样”提供，作者不作任何明示或暗示的保证，包括但不限于对适销性
 * 和特定用途适用性的暗示保证。在任何情况下，无论是在合同责任、严格责任还是
 * 侵权（包括疏忽或其他）理论下，作者均不对因使用本软件而产生的任何直接、间接、
 * 附带、特殊、示范性或继发性损害（包括但不限于采购替代物资或服务；使用损失；
 * 数据丢失；利润损失；业务中断）承担责任，即使已被告知发生此类损害的可能性。
 ***********************************************************************/

/*
在大数据和机器学习领域，高效的近邻搜索（Nearest Neighbor Search, NNS）算法是至关重要的。
nanoflann 是一个轻量级、高度优化的 C++11 库，专为计算密集型任务设计，
用于在高维空间中进行 Kd 树和球树的快速构建与查询。其优势在于速度和内存效率，
适合嵌入式系统或资源有限的环境。

nanoflann 支持两种数据结构：Kd 树和球树。Kd 树是一种分割三维或多维空间的方法，
适用于均匀分布的数据；而球树则更适合非均匀分布的数据。
这两种数据结构通过分治策略将大问题分解为小问题，从而加速查询过程。点云算法（如ICP）中最耗时的部分是查询最近邻居的KD树
————————————————
*/
