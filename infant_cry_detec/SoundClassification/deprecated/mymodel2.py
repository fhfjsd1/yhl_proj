...
class ESDB(nn.Module):
    def __init__(self, in_channels, out_channels, conv=BSConvU, p=0.25):
...
class BSRNLayer(nn.Module):
    def __init__(self,  num_feat=64, num_block=4, num_out_ch=3, upscale=4,
                 conv=BSConvU, p=0.25):
...
if __name__ == '__main__':
    ...
    flops, params = profile(model2, inputs=(torch.randn(1, 64, 224, 224),))
    print(f"FLOPs: {flops/10**9:.2f}G, Params: {params/10**6:.2f}M")
