...
class UrbanSound8kBrain(sb.core.Brain):
    ...
    def compute_forward(self, batch, stage):
        ...
        # if stage == sb.Stage.VALID and hasattr(self.hparams, "wav_augment"):
        #     wavs, lens = self.hparams.wav_augment(wavs, lens)
        ...
        # 在第二个维度上增加通道数
        feats = feats.unsqueeze(1).repeat(1, 3, 1, 1)  # 输出形状为 [16, 3, 128, 501]
        ...
        tra = torchvision.transforms.Compose([
            ...
            # torchvision.transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))  # 像素归一化
        ])
        feats = tra(feats)
        ...
        # for i in range(self.hparams.batch_size):
        #      # 创建图形对象，指定图形大小
        #     plt.figure(figsize=(pngfeat.shape[-1] / 50, pngfeat.shape[-2] / 50), dpi=100)
        #     if pngfeat.ndim == 4:
        #         librosa.display.specshow(pngfeat[i][0], sr=16000, fmax=8000)
        #     else:
        ...
        #     plt.savefig(f'mypng/spectrum{i}.png',bbox_inches='tight', pad_inches=0) # 保存图像
        ...
        # classifieembeddingsr_outputs = self.modules.classifier(embeddings)
    ...
    def compute_objectives(self, predictions, batch, stage): # predictions是forward的返回值
        ...
        #     smooth_labels = (1 - self.hparams.label_smoothing) * F.one_hot(classid, num_classes) + self.hparams.label_smoothing / num_classes
...
if __name__ == "__main__":
    ...
    f"{hparams['data_folder']}/metadata/{hparams['csv_name']}", dtype={"class_string": str, "ID": str}
        )
    # 过滤fold列数据为1, 2, 3和4的数据
    train_metadata = train_metadata.query("fold in [1, 2, 3, 4]")
    ...
    train_metadata.set_index("Name").to_json(hparams["train_annotation"], orient="index")
    
    # 从csv读取所有的val的标签数据
    val_metadata = pd.read_csv(
    f"{hparams['data_folder']}/metadata/{hparams['csv_name']}", dtype={"class_string": str, "ID": str}
        )
    val_metadata = val_metadata.query("fold in [5]")
    #train_metadata = metadata.loc[metadata["split"] == "train"].copy()
    display(
    val_metadata.head()
