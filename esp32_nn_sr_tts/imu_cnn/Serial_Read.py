...
DEF_BAUD_RATE = 921600 #1024*6*150
...
def Serial_init():
    ...
    return serial.Serial(port=port_avaliable[index - 1], # 返回与串口设备通信的连接对象
                                baudrate=DEF_BAUD_RATE, # 数据位为 8 位、无奇偶校验位以及 1 个停止位
...
# 寻找特定动作相关的文件中最大的数字，用于统计已经录制的数据集数量。
